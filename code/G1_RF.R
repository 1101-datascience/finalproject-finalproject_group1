...
for(i in 1:k){
  ...
  null_AUC_Training[i] <- performance(prediction(rbinom(dim(myTrainingData)[1], 1, 0.5),myTrainingData$TAIEX..t.),"auc")@y.values[[1]]
  ...
  myPrecision_Training[i] <- myTable[2,2]/sum(myTable[2,])
    ...
    #null model
  null_myTable <- table(as.factor(factor(rbinom(dim(myTrainingData)[1], 1, 0.5),levels=0:1)), as.factor(myTrainingData$TAIEX..t.))
  ...
  myPrecision_Validating[i] <- myTable[2,2]/sum(myTable[2,])
   ...
   #null model
  null_myTable <- table(as.factor(factor(rbinom(dim(myValidatingData)[1], 1, 0.5),levels=0:1)), as.factor(myValidatingData$TAIEX..t.))
...
myPrediction_rf <- predict(rf_model_varImp, newdata = myTest_varImp_, 'prob')
