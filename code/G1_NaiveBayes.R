...
for(i in 1:k){
  ...
  ##################### Train
  # NaiveBayes No date
  nb_model <- naiveBayes(as.factor(train_set[,7]) ~., data=train_set[,1:6])
  model_Pred <- predict(nb_model, train_set[,1:6])
  res <- as.numeric(as.character(model_Pred))
  train_Matrix <- table(train_set$TAIEX,res)
  ...
  ##################### validation
  model_Pred <- predict(nb_model, Validation_set[,1:6])
  validation_label <- Validation_set$TAIEX
  validation_Matrix <- table(validation_label,model_Pred)
...
final_NaiveBaye <- rbind(output,null_output)
...
write.csv(final_NaiveBaye, file = 'G1_NaiveBayes.csv', row.names = F, quote = F)
