...
myData_norm <- as.data.frame(lapply(myTrain[1:11], norm))
myData_norm <- cbind(myData_norm,myTrain[12:19])
...
selVars <- names(sort(varImp[,1], decreasing=T))[1:10]
myData_varImp_ <- d_[,selVars]
myData_varImp_ <- cbind(myData_varImp_,d_[19])

myTest_varImp_ <- myTest_norm[,selVars]
myTest_varImp_ <- cbind(myTest_varImp_,myTest_norm[19])

fsel <- randomForest(factor(TAIEX..t.) ~ ., data = myData_varImp_, importance=TRUE, ntree=1000, nodesize=7)
myPrediction_rf <- predict(fsel, newdata = myData_varImp_, 'prob')
accuracyMeasures(myPrediction_rf[,2], myData_varImp_$TAIEX..t., name="random forest, test")

myPrediction_rf <- predict(fsel, newdata = myTest_varImp_, 'prob')
accuracyMeasures(myPrediction_rf[,2], myTest_varImp_$TAIEX..t., name="random forest, test")


##################list <- c(16,7,3,8,1,11,2,4,5,6,23)
list <- c(16, 9, 10, 11, 2, 18, 19)
myTrain_byAcc <- d_[list]
myTest_byAcc <- myTest_norm[list]

rf_model_varImp <- randomForest(factor(TAIEX..t.) ~ ., data = myTrain_byAcc,importance=TRUE, ntree=1000, nodesize=7)
myPrediction_rf <- predict(rf_model_varImp, newdata = myTrain_byAcc, 'prob')
accuracyMeasures(myPrediction_rf[,2], myTrain_byAcc$TAIEX..t., name="random forest, test")

myPrediction_rf <- predict(rf_model_varImp, newdata = myTest_byAcc, 'prob')
accuracyMeasures(myPrediction_rf[,2], myTest_byAcc$TAIEX..t., name="random forest, test")

png(filename="varImp_rf_reTrain.png")
varImp <- importance(rf_model_varImp)
head(varImp)
varImpPlot(fsel, type=1)
dev.off()
